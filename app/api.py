...
def sqrt(op_1):
    try:
        ...
        if num_1 < 0:
            return (
                {"error": "La raíz cuadrada de números negativos no está definida"},
                http.client.BAD_REQUEST,
                HEADERS
            )
        result = CALCULATOR.sqrt(num_1)
        return (
            {"result": result},
            http.client.OK,
            HEADERS
...
def log10(op_1):
    try:
        ...
        if num_1 <= 0:
            return (
                {"error": "El logaritmo de números no positivos no está definido"},
                http.client.BAD_REQUEST,
                HEADERS
            )
        result = CALCULATOR.log10(num_1)
        return (
            {"result": result},
            http.client.OK,
            HEADERS
